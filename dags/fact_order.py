...
# global var
db_table_name = 'order_stg'
dw_table_name = 'fact_order'
...
def elt_dim_product():
    ...
    def transform_and_load_task():
        ...
        schema_table_bigquery = 'ingested_at:TIMESTAMP \
            ...
            ,city_shipping_id:INTEGER\
            ,total_product_order_quantity:INTEGER\
            ,total_price:NUMERIC\
            ,total_cost:NUMERIC\
        '  
        dedup_unique_key = 'order_id'  
        ...
        with beam.Pipeline(options=options) as p:
            read_convert_remove = ( 
                ...
                # | 'TransformData' >> ParDo(ConvertTypeOne())
                | 'ConvertTypeFloatToDecimal' >> beam.ParDo(mo.ConvertFloatToDecimal(['total_price','total_cost']))
                | 'RemoveDuplicates' >> mo.RemoveDuplicates(dedup_unique_key)
                # | 'Print Element Type' >> beam.Map(print)
            )
            write_to_bigquery = ( 
                read_convert_remove
